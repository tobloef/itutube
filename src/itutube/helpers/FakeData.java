...
public class FakeData {
    private final static String[] fakeNames = {"Mario Speedwagon", "Petey Cruiser", "Anna Sthesia", "Anna Mull", "Gail Forcewind", "Nick R. Bocker", "Mike Hunt", "Don Stairs"};
    private final static String[] fakeRoles = {"Directors", "Actors", "Cameramen", "Audio"};
    ...
    /**
     * Generates random fake credits from fake names and roles, to be used by media.
     * @return Array of instances of credits.
     */
    public static List<Credits> generateFakeCredits() {
    ...
    public static List<Media> generateRandomListOfMedia(List<Media> media, int min, int max) {
        ...
        for (int i = 0; i < amount; i++) {
            if (possibleMedia.size() == 0) {
                continue;
            }
            int randomIndex = rand.nextInt(possibleMedia.size());
            listOfMedia.add(possibleMedia.get(randomIndex));
            possibleMedia.remove(randomIndex);
        }
        return listOfMedia;
    }
